...
=== "Release"
    ...
    1. InputHandBoth (or `-1`) is a short-hand way of calling `ListenToButton` twice (once for both hands). It returns **two** IDs instead of one (See [Cancelling Listeners](#cancelling-listeners)).
...
=== "Single axis"
    ...
    1. `ANALOG_INPUT_TRIGGER_PULL` only has one axis (x axis) so we can supply a simple number value. Here we are listening to the trigger being pulled `>=` to `0.5`.
...
!!! note
    At the moment there is no way to listen for both `"up"` and `"down"` separately on different axes for an action. If you want this behaviour you will either need two listener cooperating with each other or write the logic in your own `think` using `Player:GetAnalogActionPositionForHand()`.
...
=== "With context"
    ...
    1. If `thisEntity.method` is set to a different function reference at some point this will no longer work. You must supply the same method and context values that you passed into the listener.
...
Global accessor: `Input`


--8<-- "reference/controls/input.md"